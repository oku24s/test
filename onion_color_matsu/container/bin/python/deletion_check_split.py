...
if target_name == "DFR":
    ...
    #検出閾値リード数の設定
    del_out_list = []
    for x in [indel180_list, indel466_list, indel488_list, indel1205_list, indel1992_list]:
        y = len(x) / sum_read
        if y > 0.5:
            del_out_list.append(x)

    if len(del_out_list) == 1:
        Hap1_list = del_out_list[0]
    elif len(del_out_list) >= 2:
        Hap1_list = del_out_list[0]
        Hap2_list = del_out_list[1]
